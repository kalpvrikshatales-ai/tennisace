  ...
  return (
    <div className="mb-3">
      <div className="flex justify-between text-[11px] text-white/40 mb-1">
        ...
        <span className="font-bold text-white">{v2}</span>
      </div>
  ...
  const PlayerCard = ({ player, side }: { player: any; side: 'left' | 'right' }) => {
    if (!player) return (
      <div className="flex-1 glass rounded-2xl p-4 flex items-center justify-center min-h-[140px]">
        <p className="text-white/30 text-sm text-center">Select<br/>player</p>
      </div>
    )
    ...
    return (
      <div className={`flex-1 glass rounded-2xl p-4 ${side === 'right' ? 'text-right' : ''}`}>
        ...
        <p className="text-base font-black text-white leading-tight">{player.player_full_name}</p>
        <p className="text-[11px] text-white/40 mt-0.5">{getFlag(player.player_country)} {player.player_country}</p>
        {stats && (
          <div className={`mt-2 flex gap-3 ${side === 'right' ? 'justify-end' : ''}`}>
            <span className="text-[#00C875] text-sm font-bold">#{stats.rank}</span>
            <span className="text-white/40 text-sm">{stats.matches_won}W {stats.matches_lost}L</span>
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        <p className="text-[11px] text-white/30 uppercase tracking-widest mb-4">Player vs Player</p>
        ...
        <div className="flex gap-3 mb-5">
          ...
          <div className="flex-1">
            <select
              className="w-full glass rounded-xl px-3 py-2.5 text-sm text-white bg-transparent border-0 outline-none appearance-none cursor-pointer"
              value={p1Key ?? ''}
              onChange={e => setP1Key(Number(e.target.value) || null)}
            >
              <option value="" className="bg-[#0F2A4A]">Select player 1</option>
              {rankings.map(r => (
                <option key={r.player_key} value={r.player_key} className="bg-[#0F2A4A]">
                  #{r.place} {r.player}
                </option>
              ))}
            </select>
          ...
          <div className="flex-1">
            ...
            >
              <option value="" className="bg-[#0F2A4A]">Select player 2</option>
        ...
        {loading && (
          <div className="text-center py-8 text-white/30 text-sm">Loading comparison...</div>
        )}
          ...
          <>
            ...
            {h2h.length > 0 && (
              <div className="glass rounded-2xl p-5 mb-4">
                <p className="text-[11px] text-white/30 uppercase tracking-widest mb-4">Head to Head — {h2h.length} meetings</p>
                <div className="flex items-center justify-center gap-6 mb-4">
                  <div className="text-center">
                    <p className="text-4xl font-black text-[#00C875]">{p1wins}</p>
                    <p className="text-[11px] text-white/40 mt-1">{p1?.player_full_name?.split(' ').pop()}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-bold text-white/20">–</p>
                  </div>
                  <div className="text-center">
                    <p className="text-4xl font-black text-white/60">{p2wins}</p>
                    <p className="text-[11px] text-white/40 mt-1">{p2?.player_full_name?.split(' ').pop()}</p>
                  </div>
                </div>
                  ...
                  {h2h.slice(0, 3).map((m: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between text-xs py-1.5 border-b border-white/[0.04] last:border-0">
                        <span className={p1won ? 'text-white font-semibold' : 'text-white/40'}>{m.event_first_player}</span>
                        <span className="text-white/30 text-[10px]">{m.tournament_name} {m.tournament_season}</span>
                        <span className={!p1won ? 'text-white font-semibold' : 'text-white/40'}>{m.event_second_player}</span>
                      </div>
                    )
                  })}
                </div>
            ...
            <div className="glass rounded-2xl p-5 mb-4">
              <p className="text-[11px] text-white/30 uppercase tracking-widest mb-4">2026 Season Comparison</p>
        ...
        {!p1Key && !p2Key && (
          <div className="text-center py-12">
            ...
            <p className="text-white/50 text-sm">Pick two players to compare their stats, H2H record, and surface performance.</p>
          </div>
        )}
      </main>
    </div>
