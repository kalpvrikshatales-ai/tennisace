...
export default function LiveMatchDetail({ matchId, initialMatch }: Props) {
    ...
    <div className="min-h-screen">
      <header className="sticky top-0 z-20">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href="/" className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400">
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-3">
        {[48, 32, 24].map(h => (
          <div key={h} className={`rounded-xl animate-pulse bg-gray-100`} style={{ height: h }} />
        ))}
      </div>
    </div>
  )
  ...
  if (!match) return (
    <div className="min-h-screen">
      ...
      <div className="text-center py-24 px-4">
        ...
        <Link href="/" className="mt-5 inline-block text-[#00C875] text-sm font-semibold">← Go back</Link>
      </div>
    </div>
  )
  ...
  const sv  = (key: number, name: string) =>
    stats.find((s: any) => s.player_key === key && s.stat_name === name)?.stat_value ?? '—'
  const sw  = (key: number, name: string) =>
    parseInt(stats.find((s: any) => s.player_key === key && s.stat_name === name)?.stat_won  || '0')
  const st  = (key: number, name: string) =>
    parseInt(stats.find((s: any) => s.player_key === key && s.stat_name === name)?.stat_total || '0')
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-20">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href="/" className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400 transition-colors flex-shrink-0">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
              <path d="M19 12H5M12 5l-7 7 7 7"/>
            </svg>
          </Link>
        ...
        <div className="max-w-2xl mx-auto px-5 pt-4 pb-5">
          ...
          <div className="space-y-4">
            {players.map((p, rowIdx) => {
              ...
              const isWinning = isFinished ? setsWon > setsLost : setsWon >= setsLost
              ...
              return (
                <div key={rowIdx} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={p.pkey ? `/players/${p.pkey}` : '#'} className="block group">
                    ...
                    <div className="flex items-center gap-3">
                      {setData.map((s, si) => (
                        <span key={si} className={`text-[26px] font-black tabular-nums w-7 text-right leading-none ${
                          s.me > s.opp ? 'text-white' : isFinished ? 'text-white/25' : 'text-white/50'
                        }`}>
                          {s.me}
                        </span>
                      ))}
              ...
              <div className="card p-5">
                ...
                <div className="flex h-1.5 rounded-full overflow-hidden bg-gray-100">
                  <div className="h-full rounded-l-full transition-all" style={{
                    width: `${Math.round(h2hWins1 / (h2h.length || 1) * 100)}%`, background: '#00C875',
                  }} />
