...
function CourtLines() {
  return (
    <svg style={{ position:'absolute', inset:0, width:'100%', height:'100%', opacity:0.2, pointerEvents:'none', color:'var(--sr-accent)' }}
      viewBox="0 0 1920 200" preserveAspectRatio="xMidYMid slice">
      <rect x="160" y="16" width="1600" height="168" fill="none" stroke="currentColor" strokeWidth="2"/>
      <line x1="160" y1="100" x2="1760" y2="100" stroke="currentColor" strokeWidth="1.5"/>
      <line x1="960" y1="16"  x2="960"  y2="184" stroke="currentColor" strokeWidth="1.5"/>
      <line x1="560" y1="16"  x2="560"  y2="184" stroke="currentColor" strokeWidth="1"/>
      <line x1="1360" y1="16" x2="1360" y2="184" stroke="currentColor" strokeWidth="1"/>
      <line x1="960" y1="0"   x2="960"  y2="200" stroke="currentColor" strokeWidth="3" strokeDasharray="6 4" opacity="0.6"/>
    </svg>
  )
}
    ...
    <div>
        ...
        <table style={{ borderCollapse:'collapse', minWidth:340 }}>
          <thead>
            <tr>
              <td style={{ width:50 }} />
              {DAYS.map(d => (
                <th key={d} style={{ color:'var(--sr-muted)', fontSize:11, fontWeight:700, padding:'4px 5px', textAlign:'center', letterSpacing:0.3 }}>
                  {DAY_LABEL[d]}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {TIMES.map(t => (
              <tr key={t}>
                <td style={{ color:'var(--sr-muted)', fontSize:10, fontWeight:700, paddingRight:6, textAlign:'right', whiteSpace:'nowrap', textTransform:'uppercase', letterSpacing:0.4 }}>
                  {TIME_LABEL[t]}
                </td>
                {DAYS.map(d => {
                  ...
                  return (
                    <td key={d} style={{ padding:'3px 4px', textAlign:'center' }}>
                      <div style={{
                        ...
                        background: on ? 'var(--sr-success)' : 'var(--sr-input)',
      ...
      {isOwn && (
        <button onClick={onEdit}
          style={{ marginTop:10, background:'none', border:'none', color:'var(--sr-accent)', fontSize:12, fontWeight:700, padding:'4px 0', cursor:'pointer' }}>
          Edit availability →
        </button>
      )}
    </div>
      ...
      <table style={{ borderCollapse:'collapse', minWidth:300 }}>
        <thead>
          <tr>
            <td style={{ width:44 }} />
            {DAYS.map(d => (
              <th key={d} style={{ color:'var(--sr-muted)', fontSize:10, fontWeight:700, padding:'4px 4px', textAlign:'center' }}>
                {DAY_LABEL[d]}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {TIMES.map(t => (
            <tr key={t}>
              <td style={{ color:'var(--sr-muted)', fontSize:9, fontWeight:700, paddingRight:5, textAlign:'right', textTransform:'uppercase', letterSpacing:0.4 }}>
                {TIME_LABEL[t]}
              </td>
              {DAYS.map(d => {
                const on = set.has(`${d}-${t}`)
                return (
                  <td key={d} style={{ padding:'2px 3px', textAlign:'center' }}>
                    <button onClick={() => toggle(d,t)}
                      style={{ width:26, height:26, borderRadius:5, border:`1px solid ${on ? 'var(--sr-accent)' : 'var(--sr-border)'}`, background:on ? 'var(--sr-success)' : 'var(--sr-input)', cursor:'pointer' }} />
                  </td>
                )
              })}
            </tr>
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.88)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200, padding:16 }}
      ...
      <div style={{ background:'var(--sr-card)', border:'1px solid var(--sr-border)', borderRadius:14, padding:24, maxWidth:440, width:'100%', maxHeight:'90vh', overflowY:'auto' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          <p style={{ color:'var(--sr-text)', fontWeight:800, fontSize:16, margin:0 }}>{field.label}</p>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {(field.options??[]).map(opt => (
              <button key={opt} onClick={() => setVal(opt)}
                style={{ background:val===opt ? 'var(--sr-success)' : 'var(--sr-input)', border:`1px solid ${val===opt ? 'var(--sr-accent)' : 'var(--sr-border)'}`, borderRadius:8, color:val===opt ? 'var(--sr-accent)' : 'var(--sr-text-2)', fontWeight:700, fontSize:14, padding:'12px 16px', textAlign:'left', cursor:'pointer', textTransform:'capitalize' }}>
                {opt}
              </button>
            ))}
          </div>
            ...
            {SURFACE_OPTS.map(s => {
              ...
              return (
                <button key={s} onClick={() => setVal(on ? arr.filter(x=>x!==s) : [...arr,s])}
                  style={{ background:on ? 'var(--sr-success)' : 'var(--sr-input)', border:`1px solid ${on ? 'var(--sr-accent)' : 'var(--sr-border)'}`, borderRadius:8, color:on ? 'var(--sr-accent)' : 'var(--sr-text-2)', fontWeight:700, fontSize:14, padding:'10px 18px', cursor:'pointer' }}>
                  {s}
                </button>
              )
            })}
        ...
        {field.type === 'text' && (
          <input value={val??''} onChange={e => setVal(e.target.value)}
            style={{ width:'100%', background:'var(--sr-input)', border:'1px solid var(--sr-border)', borderRadius:8, color:'var(--sr-text)', fontSize:15, padding:'12px 14px', outline:'none', boxSizing:'border-box' }} />
        )}
        ...
        {field.type === 'textarea' && (
          <textarea value={val??''} onChange={e => setVal(e.target.value)} rows={4}
            style={{ width:'100%', background:'var(--sr-input)', border:'1px solid var(--sr-border)', borderRadius:8, color:'var(--sr-text)', fontSize:15, padding:'12px 14px', outline:'none', resize:'vertical', boxSizing:'border-box', lineHeight:1.5 }} />
        )}
        ...
        <div style={{ display:'flex', gap:8, marginTop:20 }}>
          <button onClick={onClose}
            style={{ flex:1, background:'var(--sr-input)', border:'1px solid var(--sr-border)', borderRadius:8, color:'var(--sr-text-2)', fontWeight:700, fontSize:14, padding:'12px', cursor:'pointer', minHeight:44 }}>
            Cancel
          </button>
          <button onClick={() => onSave(val)} disabled={saving}
            style={{ flex:2, background:saving ? 'var(--sr-success)' : 'var(--sr-accent)', border:'none', borderRadius:8, color:saving ? 'var(--sr-succ-t)' : 'var(--sr-on-acc)', fontWeight:800, fontSize:14, padding:'12px', cursor:saving ? 'not-allowed' : 'pointer', minHeight:44 }}>
            {saving ? 'Saving…' : 'Save'}
          </button>
        </div>
      </div>
...
function PartnersTab({ profileId }: { profileId: string }) {
  ...
  if (loading) return <p style={{ color:'var(--sr-muted)', fontSize:14, textAlign:'center', padding:'40px 0' }}>Loading…</p>

  if (partners.length === 0) {
    return (
      <div style={{ textAlign:'center', padding:'56px 16px' }}>
        <p style={{ fontSize:40, margin:'0 0 14px' }}>🎾</p>
        <p style={{ color:'var(--sr-text)', fontWeight:800, fontSize:16, margin:'0 0 8px' }}>No partners yet</p>
        <p style={{ color:'var(--sr-muted)', fontSize:14, lineHeight:1.6, margin:'0 0 20px', maxWidth:280, marginLeft:'auto', marginRight:'auto' }}>
          Your next favourite hitting partner is one request away. Browse players and send your first request.
        </p>
        <a href="/sparring" style={{ display:'inline-block', background:'var(--sr-accent)', color:'var(--sr-on-acc)', fontWeight:800, fontSize:13, padding:'10px 20px', borderRadius:10, textDecoration:'none' }}>
          Find a Partner →
        </a>
      </div>
    )
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      {partners.map((p, i) => (
        <div key={i} style={{ display:'flex', alignItems:'center', gap:12, background:'var(--sr-card)', border:'1px solid var(--sr-border)', borderRadius:12, padding:'12px 14px' }}>
          <div style={{ width:44, height:44, borderRadius:'50%', background:'var(--sr-accent)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, fontWeight:900, color:'var(--sr-on-acc)', flexShrink:0 }}>
            {(p.name??'?')[0].toUpperCase()}
          </div>
          <div>
            <p style={{ color:'var(--sr-text)', fontWeight:800, fontSize:14, margin:'0 0 1px' }}>{p.name}</p>
            <p style={{ color:'var(--sr-muted)', fontSize:12, margin:'0 0 2px' }}>{p.city}</p>
            <p style={{ color:'var(--sr-accent)', fontSize:11, fontWeight:700, margin:0 }}>Tennis Partner</p>
          </div>
        </div>
      ))}
    </div>
...
function RequestsTab({ email }: { email?: string }) {
  ...
  return (
    <div>
      ...
      {received.length === 0
        ? <p style={{ color:'var(--sr-muted)', fontSize:14, marginBottom:28 }}>When someone wants to hit with you, they'll appear here.</p>
        : received.map(req => {
            const s = STATUS[req.status] ?? STATUS.pending
            return (
              <div key={req.id} style={{ background:'var(--sr-card)', border:'1px solid var(--sr-border)', borderRadius:12, padding:'14px 16px', marginBottom:10 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6 }}>
                  <div>
                    <p style={{ color:'var(--sr-text)', fontWeight:800, fontSize:14, margin:'0 0 2px' }}>{req.requester_name}</p>
                    <p style={{ color:'var(--sr-muted)', fontSize:12, margin:0 }}>{req.requester_city} · {timeAgo(req.created_at)}</p>
                  </div>
                  <span style={{ background:s.bg, color:s.color, fontSize:11, fontWeight:800, padding:'3px 8px', borderRadius:5, flexShrink:0 }}>{s.label}</span>
                </div>
                {req.status==='accepted' && req.from_phone && (
                  <div style={{ background:'var(--sr-success)', border:'1px solid var(--sr-succ-t)', borderRadius:8, padding:'8px 12px', marginTop:8 }}>
                    <p style={{ color:'var(--sr-muted)', fontSize:10, margin:'0 0 2px', textTransform:'uppercase', fontWeight:700 }}>Their number</p>
                    <p style={{ color:'var(--sr-succ-t)', fontSize:15, fontWeight:800, margin:0 }}>{req.from_phone}</p>
                  </div>
                )}
                {req.status==='pending' && (
                  <div style={{ display:'flex', gap:8, marginTop:10 }}>
                    <button onClick={()=>decline(req.id)} disabled={!!acting}
                      style={{ flex:1, background:'var(--sr-input)', border:'1px solid var(--sr-border)', borderRadius:8, color:'var(--sr-text-2)', fontWeight:700, fontSize:13, padding:'10px', cursor:acting?'not-allowed':'pointer', minHeight:44 }}>
                      Decline
                    </button>
                    <button onClick={()=>accept(req.id)} disabled={!!acting}
                      style={{ flex:2, background:acting===req.id ? 'var(--sr-success)' : 'var(--sr-accent)', border:'none', borderRadius:8, color:acting===req.id ? 'var(--sr-succ-t)' : 'var(--sr-on-acc)', fontWeight:800, fontSize:13, padding:'10px', cursor:acting?'not-allowed':'pointer', minHeight:44 }}>
      ...
      <p style={{ color:'var(--sr-muted)', fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1, margin:'28px 0 12px' }}>
        Sent ({sent.length})
      </p>
      {sent.length === 0
        ? <p style={{ color:'var(--sr-muted)', fontSize:14 }}>No requests sent yet.</p>
        : sent.map(req => {
            const s = STATUS[req.status] ?? STATUS.pending
            ...
            return (
              <div key={req.id} style={{ background:'var(--sr-card)', border:'1px solid var(--sr-border)', borderRadius:12, padding:'14px 16px', marginBottom:10 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div>
                    <p style={{ color:'var(--sr-text)', fontWeight:800, fontSize:14, margin:'0 0 2px' }}>{p?.name ?? '—'}</p>
                ...
                {req.status==='accepted' && p?.phone && (
                  <div style={{ background:'var(--sr-success)', border:'1px solid var(--sr-succ-t)', borderRadius:8, padding:'8px 12px', marginTop:8 }}>
                    <p style={{ color:'var(--sr-muted)', fontSize:10, margin:'0 0 2px', textTransform:'uppercase', fontWeight:700 }}>Their number</p>
                    <p style={{ color:'var(--sr-succ-t)', fontSize:15, fontWeight:800, margin:0 }}>{p.phone}</p>
                  </div>
                )}
              </div>
            )
...
export default function SparringProfilePage() {
  ...
  async function uploadImage(type: 'cover'|'avatar', file: File) {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND}/sparring/profiles/${id}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(type === 'cover' ? { cover_url: data.publicUrl } : { photo_url: data.publicUrl }),
      })
    ...
    return (
      <SparringShell>
        <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
          <p style={{ color:'var(--sr-muted)', fontSize:16 }}>{error || 'Profile not found.'}</p>
          <Link href="/sparring" style={{ color:'var(--sr-accent)', fontSize:14 }}>← Back to Find a Partner</Link>
        </div>
      </SparringShell>
    )
  }
      ...
      <div className="sr-page">
        ...
        <div style={{
          ...
          <button onClick={() => router.back()}
            style={{ position:'absolute', top:14, left:14, background:'rgba(0,0,0,0.5)', border:'1px solid var(--sr-border)', borderRadius:8, color:'var(--sr-text-2)', fontSize:13, fontWeight:700, padding:'7px 13px', cursor:'pointer', backdropFilter:'blur(6px)' }}>
            ← Back
          </button>
          ...
          {isOwn && (
            <UploadBtn onFile={f => uploadImage('cover', f)}
              ...
              <div style={{ background:'rgba(0,0,0,0.6)', border:'1px solid var(--sr-border)', borderRadius:8, color:'var(--sr-text-2)', fontSize:12, fontWeight:700, padding:'7px 12px', backdropFilter:'blur(6px)', display:'flex', alignItems:'center', gap:6 }}>
                {uploading==='cover'
                  ? <div style={{ width:14, height:14, borderRadius:'50%', border:'2px solid var(--sr-border)', borderTopColor:'var(--sr-accent)', animation:'spin 0.7s linear infinite' }} />
                  : <>✏️ Edit cover</>}
              </div>
            </UploadBtn>
          )}
        ...
        <div style={{ maxWidth:640, margin:'0 auto', padding:'0 16px' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginTop:-48, paddingBottom:14 }}>
            <div style={{ position:'relative', flexShrink:0 }}>
              <div style={{
                width:96, height:96, borderRadius:'50%', border:'4px solid var(--sr-bg)',
                background: profile.photo_url ? `url(${profile.photo_url}) center/cover no-repeat` : 'var(--sr-accent)',
                display:'flex', alignItems:'center', justifyContent:'center',
                fontSize:36, fontWeight:900, color:'var(--sr-on-acc)', overflow:'hidden',
                position:'relative',
              }}>
                ...
                {uploading === 'avatar' && (
                  <div style={{ position:'absolute', inset:0, borderRadius:'50%', background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <div style={{ width:24, height:24, borderRadius:'50%', border:'3px solid var(--sr-border)', borderTopColor:'var(--sr-accent)', animation:'spin 0.7s linear infinite' }} />
                  </div>
                )}
              </div>
              {isOwn && (
                <UploadBtn onFile={f => uploadImage('avatar', f)} style={{ position:'absolute', bottom:2, right:2 }}>
                  <div style={{ width:26, height:26, borderRadius:'50%', background:'var(--sr-card)', border:'2px solid var(--sr-bg)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12 }}>📷</div>
                </UploadBtn>
              )}
            </div>
              ...
              {isOwn ? (
                <>
                  <Link href={`/sparring/edit/${id}`}
                    style={{ background:'transparent', border:'1px solid var(--sr-border)', borderRadius:10, color:'var(--sr-text-2)', fontWeight:700, fontSize:13, padding:'9px 14px', textDecoration:'none', whiteSpace:'nowrap', minHeight:44, display:'flex', alignItems:'center' }}>
                    Settings
                  </Link>
                  <Link href={`/sparring/edit/${id}`}
                    style={{ background:'var(--sr-accent)', border:'none', borderRadius:10, color:'var(--sr-on-acc)', fontWeight:800, fontSize:13, padding:'9px 16px', textDecoration:'none', whiteSpace:'nowrap', minHeight:44, display:'flex', alignItems:'center' }}>
                    Edit Profile
                  </Link>
                </>
              ) : (
          ...
          {/* Upload error */}
          {uploadError && (
            <div style={{ background:'var(--sr-danger)', border:'1px solid var(--sr-dang-t)', borderRadius:8, padding:'8px 14px', marginBottom:12, display:'flex', alignItems:'center', justifyContent:'space-between', gap:10 }}>
              <span style={{ color:'var(--sr-dang-t)', fontSize:13 }}>{uploadError}</span>
              <button onClick={() => setUploadError(null)} style={{ background:'none', border:'none', color:'var(--sr-dang-t)', cursor:'pointer', fontSize:16, lineHeight:1, padding:2 }}>×</button>
            </div>
          )}
          ...
          {isOwn && (() => {
            ...
            return (
              <div style={{ marginBottom:18 }}>
                <div style={{ background:'var(--sr-border)', borderRadius:2, height:4, overflow:'hidden' }}>
                  <div style={{ width:`${pct}%`, height:'100%', background:'var(--sr-accent)', borderRadius:2, transition:'width 0.5s ease' }} />
                </div>
                <p style={{ color: complete ? 'var(--sr-succ-t)' : 'var(--sr-muted)', fontSize:11, margin:'5px 0 0', fontWeight:600 }}>
            ...
            {tabs.map(tab => (
              <button key={tab.key} onClick={() => setActiveTab(tab.key as any)}
                style={{
                  ...
                  borderBottom: activeTab===tab.key ? '2px solid var(--sr-accent)' : '2px solid transparent',
                  marginBottom:-1,
                  padding:'12px 18px 11px',
                  color: activeTab===tab.key ? 'var(--sr-text)' : 'var(--sr-muted)',
