...
function GameRow({ set_number, number_game, player_served, serve_winner, score, points, player1, player2, defaultOpen }: Game & { player1: string; player2: string; defaultOpen?: boolean }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 overflow-hidden mb-2">
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-4 py-3 glass hover:bg-gray-50 transition-colors text-left"
      >
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-gray-400 font-mono w-6">{number_game}</span>
          <div className="flex items-center gap-1.5">
            {p1served && <span className="text-[10px] text-gray-500">🎾</span>}
            <span className={`text-xs font-semibold ${p1won ? 'text-white' : 'text-gray-500'}`}>
              {player1.split(' ').pop()}
            </span>
          </div>
          <span className="text-gray-300 text-xs">–</span>
          <div className="flex items-center gap-1.5">
            {p2served && <span className="text-[10px] text-gray-500">🎾</span>}
            <span className={`text-xs font-semibold ${p2won ? 'text-white' : 'text-gray-500'}`}>
              {player2.split(' ').pop()}
            </span>
          </div>
          {breakHappened && (
          ...
          {points.map((pt, i) => {
            ...
            return (
              <div key={i} className={`flex items-center justify-between py-1 px-2 rounded-lg ${isWinner ? 'bg-[#00C875]/10' : ''}`}>
                ...
                <span className={`text-xs font-bold tabular-nums font-mono ${isWinner ? 'text-[#00C875]' : 'text-gray-600'}`}>
                  {pt.score}
                </span>
              </div>
            )
  ...
  return (
    <div className="mt-5">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[11px] text-gray-400 uppercase tracking-widest">Point by Point</p>
        <div className="flex gap-1.5">
          <button
            onClick={() => setActiveSet('all')}
            className={`text-[10px] px-2 py-1 rounded-full font-semibold transition-colors ${activeSet === 'all' ? 'bg-[#00C875]/20 text-[#00C875]' : 'text-gray-400 hover:text-gray-600'}`}
          >
            All
          </button>
          {sets.map(s => (
            <button
              ...
              className={`text-[10px] px-2 py-1 rounded-full font-semibold transition-colors ${activeSet === s ? 'bg-[#00C875]/20 text-[#00C875]' : 'text-gray-400 hover:text-gray-600'}`}
            >
              S{s.replace('Set ', '')}
            </button>
          ))}
