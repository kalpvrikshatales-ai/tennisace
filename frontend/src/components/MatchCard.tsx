      ...
      <div className="flex items-center justify-between gap-2 mb-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button
            ...
            className="text-white/20 hover:text-white/60 transition-colors text-sm p-1 -mr-1"
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="flex gap-2">
              {scores.map((s, i) => {
                ...
                const p2 = parts[1] ?? '0'
                return (
                  <span key={i} className={`text-sm font-bold tabular-nums w-6 text-center ${parseInt(p1) > parseInt(p2) ? 'text-white' : 'text-white/40'}`}>
                    {p1}
                  </span>
                )
              })}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="flex gap-2">
              {scores.map((s, i) => {
                ...
                const p2 = parts[1] ?? ''
                return (
                  <span key={i} className={`text-sm font-bold tabular-nums w-6 text-center ${parseInt(p2) > parseInt(p1) ? 'text-white' : 'text-white/40'}`}>
                    {p2}
                  </span>
                )
              })}
